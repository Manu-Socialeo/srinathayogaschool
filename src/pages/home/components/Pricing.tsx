...
export default function Pricing() {
  ...
  return (
    <section ref={sectionRef} id="pricing" className="py-24 bg-gradient-to-b from-teal-50 to-white">
      <div className="container mx-auto px-4">
        <div className={`flex flex-col lg:flex-row items-center justify-between gap-8 mb-16 ${isVisible ? 'opacity-0 animate-fade-in-up' : 'opacity-0'}`}>
          <div>
            <h4 className="text-sm font-semibold text-teal-500 mb-3">Our Plans</h4>
            <h2 className="text-4xl font-bold tracking-tight">Flexible Monthly Yoga Plans</h2>
          </div>
          <p className={`text-gray-600 lg:max-w-md ${isVisible ? 'opacity-0 animate-fade-in-up animation-delay-200' : 'opacity-0'}`}>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            <div 
              key={index} 
              className={`rounded-2xl p-8 ${plan.featured ? 'bg-teal-500 text-white transform lg:-translate-y-4' : 'bg-white'} shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-500 ${
            ...
            >
              ...
              <Link 
                to="/courses" 
                className={`block text-center px-6 py-3 rounded-lg text-sm font-medium transition-all duration-300 cursor-pointer whitespace-nowrap hover:scale-105 ${
                  plan.featured 
